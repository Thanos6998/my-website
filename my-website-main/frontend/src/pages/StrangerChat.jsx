...
import { Send, X, SkipForward, Image as ImageIcon, Search, Loader, ArrowLeft, Smile, Users } from 'lucide-react';
...
const vibrate = (pattern) => {
  try { if (navigator.vibrate) navigator.vibrate(pattern); } catch (e) { /* silent */ }
};
...
const StrangerChat = () => {
  ...
  // Cleanup: mark unmounted + intentional close so reconnect logic doesn't fire
  useEffect(() => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    socket.onopen = () => {
      ...
      reconnectAttempts = 0;
      // Send immediate pong to establish bidirectional data flow (prevents proxy idle kill)
    ...
    socket.onmessage = (event) => {
      ...
      // Handle keepalive pings from server — respond with pong
      if (data.type === 'ping') {
      ...
      if (data.type === 'matched') {
        ...
        setMessages(prev => [...prev, { type: 'received', image: data.url, timestamp: Date.now() }]);
        playSound('message');
        vibrate(50);
        ...
        // Partner left — but OUR socket stays alive so user can click "Next"
        setPartnerId(null);
        setIsTyping(false);
        ...
        // Keep status as 'connected' so Next/Stop buttons remain visible
      }
    };

    socket.onclose = (event) => {
      wsRef.current = null;
      // Never reconnect if the component has unmounted or close was intentional
      if (!mountedRef.current || intentionalCloseRef.current) {
        return;
      }
      // Auto-reconnect only for unexpected closes while in an active session
      ...
      if (statusRef.current === 'searching' && !intentionalCloseRef.current) {
  ...
  // Safe WebSocket send — won't crash on dead socket
  const safeSend = (data) => {
    try {
  ...
  const handleCancel = () => {
    ...
    wsRef.current = null; setWs(null); setStatus('idle'); setMessages([]); setPartnerId(null);
    setShowGifPicker(false); setShowEmojiPicker(false);
    toast.info('Search cancelled');
  };

  const handleNext = () => {
    if (!safeSend({ action: 'next' })) {
      // Socket is dead — reconnect fresh
      if (wsRef.current) wsRef.current.close();
      wsRef.current = null;
      setMessages([]); setPartnerId(null); setStatus('idle');
      setShowGifPicker(false); setShowEmojiPicker(false); setIsTyping(false);
      toast.info('Reconnecting...');
      setTimeout(() => connectWebSocket(), 300);
      return;
    }
    setMessages([]); setPartnerId(null); setStatus('searching');
    setShowGifPicker(false); setShowEmojiPicker(false); setIsTyping(false);
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      if (safeSend({ type: 'image', url: imageUrl })) {
        setMessages(prev => [...prev, { type: 'sent', image: imageUrl, timestamp: Date.now() }]);
        toast.success('Image sent!');
      } else {
        toast.error('Connection lost. Image uploaded but could not send.');
      }
    } catch (error) {
  ...
  const renderMessage = (msg, index) => {
    if (msg.type === 'system') {
      return (
        <div key={index} className="flex justify-center" data-testid={`msg-system-${index}`}>
    ...
    return (
      <div key={index} className={`flex ${isSent ? 'justify-end' : 'justify-start'}`} data-testid={`msg-${msg.type}-${index}`}>
        <div className={`max-w-[75%] sm:max-w-md rounded-2xl p-3 ${
          ...
          {msg.image && <img src={`${API}/files/${msg.image}`} alt="Shared" className="max-w-full rounded-xl mt-1 cursor-pointer" loading="lazy" onClick={() => window.open(`${API}/files/${msg.image}`, '_blank')} />}
          {msg.gif && <img src={msg.gif} alt="GIF" className="max-w-full rounded-xl mt-1" loading="lazy" />}
        </div>
      </div>
    );
  };

  // Height of the fixed input bar area for bottom padding
  const inputBarHeight = status === 'connected' ? (showEmojiPicker || showGifPicker ? 'pb-[360px]' : 'pb-[88px]') : 'pb-4';

  return (
    <div className="h-[100dvh] flex flex-col bg-[#050505] relative" data-testid="stranger-chat-page">
      {/* Header */}
      <div className="shrink-0 bg-[#0A0808]/95 backdrop-blur-xl border-b border-[#271A1A]/60 px-3 py-2.5 sm:px-4 sm:py-3 z-30">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            <button onClick={() => { if (wsRef.current) wsRef.current.close(); navigate('/'); }}
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-1.5 bg-green-500/10 border border-green-500/20 px-3 py-1.5 rounded-full" data-testid="online-count">
      ...
      {/* Chat Area — fills space between header and fixed input bar */}
      <div className={`flex-1 overflow-y-auto p-4 ${inputBarHeight}`} data-testid="chat-messages-area">
        <div className="max-w-4xl mx-auto space-y-3">
          {status === 'idle' && (
            <div className="text-center py-16 sm:py-20" data-testid="idle-state">
              ...
              <p className="text-[#5A3A3A] mb-4 text-sm sm:text-base max-w-sm mx-auto">
                Connect with random strangers. Share text, photos, and GIFs anonymously!
              </p>
              {onlineCount > 0 && (
              ...
              <button onClick={handleStart}
                ...
                data-testid="start-chat-big-btn">
                Find a Stranger
              </button>
            </div>
          )}
          ...
          {status === 'searching' && (
            <div className="text-center py-16 sm:py-20" data-testid="searching-state">
              <div className="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 bg-[#1A1010] rounded-full mb-6 border border-[#271A1A]">
      ...
      {/* Fixed bottom bar — emoji/gif pickers + capsule input */}
      {status === 'connected' && (
        <div className="fixed bottom-0 left-0 right-0 z-40" data-testid="chat-input-area">
          {/* Emoji Picker */}
          {showEmojiPicker && (
            <div className="border-t border-[#271A1A] bg-[#0A0808]" data-testid="emoji-picker-container">
          ...
          {/* GIF Picker */}
          {showGifPicker && (
            <div className="bg-[#0A0808] border-t border-[#271A1A] h-[280px] flex flex-col" data-testid="gif-picker">
              <div className="p-2.5 border-b border-[#271A1A]">
                <div className="flex items-center gap-2">
                  <input type="text" value={gifSearch}
                    onChange={(e) => { setGifSearch(e.target.value); if (e.target.value.length > 1) searchGifs(e.target.value); else if (e.target.value.length === 0) fetchTrendingGifs(); }}
                  ...
                  <button onClick={() => setShowGifPicker(false)} className="text-[#5A3A3A] hover:text-white p-2" data-testid="close-gif-picker"><X size={20} /></button>
                </div>
              </div>
                ...
                {loadingGifs ? (
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {gifs.map((gif) => {
                      ...
                      return (
                        <button key={gif.id} onClick={() => handleSendGif(gifUrl)} className="rounded-lg overflow-hidden hover:ring-2 hover:ring-[#E63946] transition-all active:scale-95" data-testid={`gif-item-${gif.id}`}>
          ...
          {/* Capsule Input Bar */}
          <div className="bg-[#0A0808]/95 backdrop-blur-xl border-t border-[#271A1A]/40 px-3 py-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]" data-testid="capsule-input-bar">
            <div className="max-w-4xl mx-auto flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => fileInputRef.current?.click()} disabled={uploadingMedia}
                  className="p-2 rounded-full hover:bg-[#1A1010] transition-all active:scale-90 text-[#5A3A3A] hover:text-[#E63946]" data-testid="upload-btn">
                ...
                <button onClick={() => { setShowGifPicker(prev => !prev); setShowEmojiPicker(false); }}
                  className={`p-2 rounded-full hover:bg-[#1A1010] transition-all active:scale-90 ${showGifPicker ? 'text-[#E63946]' : 'text-[#5A3A3A] hover:text-[#E63946]'}`} data-testid="gif-btn">
                  <span className="text-xs font-extrabold">GIF</span>
                </button>
                <button onClick={() => { setShowEmojiPicker(prev => !prev); setShowGifPicker(false); }}
                  className={`p-2 rounded-full hover:bg-[#1A1010] transition-all active:scale-90 ${showEmojiPicker ? 'text-[#E63946]' : 'text-[#5A3A3A] hover:text-[#E63946]'}`} data-testid="emoji-btn">
                  <Smile size={20} />
                </button>
              </div>

              {/* Capsule text input */}
              <div className="flex-1 flex items-center bg-[#141010] border border-[#271A1A]/70 rounded-full px-4 py-0.5 focus-within:border-[#E63946]/50 focus-within:shadow-[0_0_12px_rgba(230,57,70,0.1)] transition-all">
