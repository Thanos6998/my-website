...
const CreateConfessionModal = ({ onClose, onSuccess }) => {
  ...
  const handleMediaChange = (e) => {
    ...
    if (file.type.startsWith('image/') && sizeMB > 2) { setError('Image max 2MB'); return; }
    if (file.type.startsWith('video/') && sizeMB > 10) { setError('Video max 10MB'); return; }
    setMedia(file);
    setError('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      params.append('is_adult', isAdult);
      if (city) params.append('city', city);
      await api.post(`/confessions?${params.toString()}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      toast.success('Whisper shared!');
      onSuccess();
      onClose();
    } catch (error) {
      setError(error.response?.data?.detail || 'Failed to post');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3">
          <button onClick={onClose} className="text-white/40 hover:text-white/70 text-sm font-medium transition-colors" data-testid="close-modal-btn">Cancel</button>
          <h2 className="text-base font-bold text-white" style={{ fontFamily: "'Playfair Display', serif" }}>New Whisper</h2>
          <button
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-6">
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            {categories.map((cat) => (
              <button key={cat.value} type="button" onClick={() => setCategory(cat.value)}
                ...
                data-testid={`category-${cat.value}`}>{cat.label}</button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between border-t border-white/[0.04] pt-4">
            <div className="flex items-center gap-1">
              <label className="flex items-center gap-1.5 text-white/35 hover:text-white/60 cursor-pointer p-2.5 rounded-full hover:bg-white/[0.05] transition-all">
                <Image size={20} /><span className="text-xs font-medium">Photo</span>
                <input type="file" accept="image/*" onChange={handleMediaChange} className="hidden" data-testid="image-upload-input" />
              </label>
              <label className="flex items-center gap-1.5 text-white/35 hover:text-white/60 cursor-pointer p-2.5 rounded-full hover:bg-white/[0.05] transition-all">
                <Video size={20} /><span className="text-xs font-medium">Video</span>
                <input type="file" accept="video/*" onChange={handleMediaChange} className="hidden" data-testid="video-upload-input" />
              </label>
            </div>
            <button type="button" onClick={() => setShowMore(!showMore)} className="text-white/35 hover:text-white/60 text-xs font-medium flex items-center gap-1 transition-colors">
          ...
          {showMore && (
            <div className="mt-4 space-y-3 animate-fade-in">
              <input type="text" value={nickname} onChange={(e) => setNickname(e.target.value)} placeholder="Custom nickname (optional)"
              ...
              <label className="flex items-center gap-3 bg-[#E63946]/[0.04] border border-[#E63946]/10 rounded-xl px-4 py-3 cursor-pointer">
                <input type="checkbox" checked={isAdult} onChange={(e) => setIsAdult(e.target.checked)} className="w-4 h-4 rounded accent-[#E63946]" data-testid="adult-content-checkbox" />
